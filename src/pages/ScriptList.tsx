      ...
      <div className="w-80 shrink-0 border-r border-border flex flex-col">
        <div className="flex items-center justify-between px-4 pt-4 pb-1">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-muted-foreground bg-muted px-1.5 py-0.5 rounded">{currentFolderName}</span>
          </div>
          <Button size="sm" onClick={() => onEditSnippet(null, folderId)}>+ 新增</Button>
        </div>

        <div className="px-3 pb-2 pt-2">
          <Input placeholder="搜索代码…" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="h-8 text-xs" />
        </div>
            ...
            filtered.map((s) => (
              <div
                ...
                className={`rounded-md px-3 py-2 cursor-pointer transition-colors ${
                  selectedId === s.id ? "bg-primary/10 border border-primary/30" : "hover:bg-muted border border-transparent"
                }`}
              >
                <div className="flex items-center justify-between gap-2">
                  <span className="text-sm font-medium truncate flex-1">{s.name}</span>
                  <Badge className={LANG_STYLES[s.language] || "bg-zinc-100 text-zinc-500"}>{langLabel(s.language)}</Badge>
                </div>
                <p className="text-xs text-muted-foreground truncate mt-0.5 font-mono">{s.content.slice(0, 80)}</p>
              </div>
            ))
          )}
        </div>
          ...
          <>
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex gap-2">
                <Button size="sm" variant="outline" onClick={() => handleCopy(selected.content)}>复制</Button>
                <Button size="sm" variant="secondary" onClick={() => onEditSnippet(selected.id, selected.folder_id)}>编辑</Button>
                <Button size="sm" variant="destructive" onClick={() => { if (confirm("确定删除？")) handleDelete(selected.id); }}>删除</Button>
              </div>
            </div>

            <div className="flex items-center gap-3 mb-3 text-xs text-muted-foreground">
              <Badge className={LANG_STYLES[selected.language] || "bg-zinc-100 text-zinc-500"}>{langLabel(selected.language)}</Badge>
              {selected.folder_id && (
                <span>📂 {folders.find((f) => f.id === selected.folder_id)?.name || "未分类"}</span>
              )}
            ...
            <div className="flex gap-2 mb-3">
              <Button size="sm" variant="secondary" onClick={() => handleCopy(selected.content)}>复制代码</Button>
              <Button size="sm" onClick={handleExecute} disabled={executing}>{executing ? "执行中…" : "▶ 运行"}</Button>
            </div>
            ...
            {result && (
              <div className="border border-border rounded-lg overflow-hidden bg-card">
                <div className="flex items-center justify-between px-3 py-2 bg-muted/50 text-xs text-muted-foreground">
                  ...
                  <Button size="sm" variant="ghost" onClick={() => setResult(null)}>关闭</Button>
                </div>
                <div className="max-h-48 overflow-y-auto divide-y divide-border">
                  {result.stdout && <div className="px-3 py-2"><p className="text-[10px] text-muted-foreground uppercase tracking-wide mb-1">标准输出</p><pre className="text-xs font-mono whitespace-pre-wrap">{result.stdout}</pre></div>}
                  {result.stderr && <div className="px-3 py-2"><p className="text-[10px] text-muted-foreground uppercase tracking-wide mb-1">错误输出</p><pre className="text-xs font-mono whitespace-pre-wrap text-destructive">{result.stderr}</pre></div>}
                  {!result.stdout && !result.stderr && <p className="text-xs text-muted-foreground text-center py-4">（无输出）</p>}
                </div>
              </div>
            )}
          </>
        ) : (
          <div className="flex items-center justify-center h-full text-sm text-muted-foreground">从左侧选择一个代码片段</div>
        )}
      </div>
    </div>
  );
